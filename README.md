...
Pure client-side -- HTML, CSS, and vanilla JavaScript served from GitHub Pages. No backend, no database, no stored data. Authentication is handled by MSAL.js with Nested App Authentication (NAA) where the host supports it. All data flows directly between the user's browser and Microsoft Graph using delegated permissions.
...
Already done for the hosted instance. Only needed if forking under a different Entra app or host.

```powershell
.\setup\ShareLynx_Entra_Setup.ps1 -MultiTenant -BaseUrl https://your-host/your-repo
```

Push to a public repo and enable GitHub Pages. `manifest.xml` points at the Pages URL.
...
Code changes are just a push. Manifest changes must be re-uploaded per tenant; rare.
